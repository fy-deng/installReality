...
curl -Lo go.tar.gz https://go.dev/dl/go1.20.3.linux-arm64.tar.gz
...
curl -o config.json https://raw.githubusercontent.com/BoxXt/installReality/main/config.json && mv config.json /etc/sing-box/config.json
curl -o meta.yaml https://raw.githubusercontent.com/BoxXt/installReality/main/meta.yaml && mv meta.yaml /etc/sing-box/meta.yaml
...
read pukey
sed -in "s/pkey/$pkey/g" /etc/sing-box/config.json

echo "生成uuid"
uuid=`sing-box generate uuid`
echo $uuid
sed -in "s/puuid/$uuid/g" /etc/sing-box/config.json
...
echo "随机生成short_id"
echo $shortid
sed -in "s/pshortid/$shortid/g" /etc/sing-box/config.json
...
sed -in "s/vpsip/$ip/g" /etc/sing-box/meta.yaml
sed -in "s/pukey/$pukey/g" /etc/sing-box/meta.yaml
sed -in "s/pshortid/$shortid/g" /etc/sing-box/meta.yaml
sed -in "s/puuid/$uuid/g" /etc/sing-box/meta.yaml
echo "\033[32m 以下是你的meta客户端所需要的可用示例配置文件： \033[0m"
echo "========================================================="
cat /etc/sing-box/meta.yaml
echo "完成搭建"
